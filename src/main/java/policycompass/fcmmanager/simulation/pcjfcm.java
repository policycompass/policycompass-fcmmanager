...
public class pcjfcm {
	...
	public static FCMSimulationDetail runFCMSimulation(JSONObject jsonModel) {
		...
		try {
			...
			for (int i = 0; i < concepts.length(); i++) {
				...
				for (int j = 0; j < conceptdb.size(); j++) {
					if (conceptdb.get(j).getId() == Integer.parseInt(ob.getString("Id").toString())) {
						Query qConceptIndividuals = session
								.createQuery("from fcmmanager_conceptindividuals where Concept_id= :id");
			...
			for (int i = 0; i < simulationConcept.size(); i++) {
				...
				switch (activatorID) {
				case 1:
					Concept cc1 = new Concept("c" + simulationConcept.get(i).getConceptID(), concept.get(i).getTitle(),
							afCauchyActivator, 0.0, simulationConcept.get(i).getScaleValue(),
							simulationConcept.get(i).getFixedOutput());
					c[i] = cc1;
					break;

				case 2:
					Concept cc2 = new Concept("c" + simulationConcept.get(i).getConceptID(), concept.get(i).getTitle(),
							afGaussianActivator, 0.0, simulationConcept.get(i).getScaleValue(),
							simulationConcept.get(i).getFixedOutput());
					c[i] = cc2;
					break;

				case 3:
					Concept cc3 = new Concept("c" + simulationConcept.get(i).getConceptID(), concept.get(i).getTitle(),
							afHyperbolicTangentActivator, 0.0, simulationConcept.get(i).getScaleValue(),
							simulationConcept.get(i).getFixedOutput());
					c[i] = cc3;
					break;

				case 4:
					Concept cc4 = new Concept("c" + simulationConcept.get(i).getConceptID(), concept.get(i).getTitle(),
							afIntervalActivator, 0.0, simulationConcept.get(i).getScaleValue(),
							simulationConcept.get(i).getFixedOutput());
					c[i] = cc4;
					break;

				case 5:
					Concept cc5 = new Concept("c" + simulationConcept.get(i).getConceptID(), concept.get(i).getTitle(),
							afLinearActivator, 0.0, simulationConcept.get(i).getScaleValue(),
							simulationConcept.get(i).getFixedOutput());
					c[i] = cc5;
					break;

				case 6:
					Concept cc6 = new Concept("c" + simulationConcept.get(i).getConceptID(), concept.get(i).getTitle(),
							afNaryActivator, 0.0, simulationConcept.get(i).getScaleValue(),
							simulationConcept.get(i).getFixedOutput());
					c[i] = cc6;
					break;

				case 7:
					Concept cc7 = new Concept("c" + simulationConcept.get(i).getConceptID(), concept.get(i).getTitle(),
							afSigmoidActivator, 0.0, simulationConcept.get(i).getScaleValue(),
							simulationConcept.get(i).getFixedOutput());
					c[i] = cc7;
					break;

				case 8:
					Concept cc8 = new Concept("c" + simulationConcept.get(i).getConceptID(), concept.get(i).getTitle(),
							afSignumActivator, 0.0, simulationConcept.get(i).getScaleValue(),
							simulationConcept.get(i).getFixedOutput());
					c[i] = cc8;
			...
			for (int i = 0; i < NumberIterations; i++) {
				for (int j = 0; j < simulationConcept.size(); j++) {
					...
					if (c[j].getOutput() == null)
						...
						// conceptOutput =
						// Double.parseDouble(df2.format(c[j].getOutput()));
	...
	public static FCMSimulationDetail runImpactAnalysis(int id, JSONObject jsonModel) {
		...
		try {
			...
			if (id == 1) {
				selectedConceptID = Integer.parseInt(
						jsonModel.getJSONObject("data").getJSONObject("selectedConcept").get("Id").toString());

				setSimulationConceptValues(listSimulationConcept, simulationConcept, selectedConceptID);
			} else {
				selectedConceptID1 = Integer.parseInt(
						jsonModel.getJSONObject("data").getJSONObject("selectedConcept1").get("Id").toString());
				selectedConceptID2 = Integer.parseInt(
						jsonModel.getJSONObject("data").getJSONObject("selectedConcept2").get("Id").toString());
				selectedConceptID3 = Integer.parseInt(
						jsonModel.getJSONObject("data").getJSONObject("selectedConcept3").get("Id").toString());

				setSimulationConceptValues(listSimulationConcept, simulationConcept, selectedConceptID1,
						selectedConceptID2);
			}
			...
			for (int x = 0; x < listSimulationConcept.size(); x++) {
				...
				for (int i = 0; i < listSimulationConcept.get(x).size(); i++) {
					...
					Concept cc = new Concept("c" + listSimulationConcept.get(x).get(i).getConceptID(),
							concept.get(i).getTitle(), af, 0.0, listSimulationConcept.get(x).get(i).getScaleValue(),
							listSimulationConcept.get(x).get(i).getFixedOutput());
					c[i] = cc;
					map.addConcept(c[i]);
				}
				...
				for (int i = 0; i < simulationConnection.size(); i++) {
					...
					FcmConnection conn = new WeightedConnection(
							"c" + connection.get(i).getConceptFrom() + " -> " + "c" + connection.get(i).getConceptTo(),
							source + " -> " + destination, simulationConnection.get(i).getWeighted());
					map.addConnection(conn);
					map.connect("c" + connection.get(i).getConceptFrom(),
							"c" + connection.get(i).getConceptFrom() + " -> " + "c" + connection.get(i).getConceptTo(),
							"c" + connection.get(i).getConceptTo());
				}
	...
	public static int getConceptIndividualID() {
		...
		session.beginTransaction();
		Criteria criteria = session.createCriteria(FCMConceptIndividual.class).setProjection(Projections.max("id"));
		if (criteria.uniqueResult() == null) {
